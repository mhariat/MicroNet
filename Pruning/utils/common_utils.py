...
def count_nonzero_parameters(module):
    return torch.gt(module.weight.data, 0).sum().item()
...
def train_sp(model, optimizer, use_cuda, train_dataloader, epoch, alpha, log_interval=100, multi_gpu=False):
...
def train(model, optimizer, use_cuda, train_dataloader, epoch, log_interval=100, multi_gpu=False):
    if multi_gpu:
        model = nn.DataParallel(model.cuda())
    model.train()
    iteration = 0
