...
class ImprovedEigenPrunerMulti(LowRankPruner):
    def __init__(self, model, prune_ratio_limit, log_interval, back_ratio=0, normalize=False, use_bias=True,
                 fisher_type='true', regularization=1e-3, ma=0.9, correct_eigenvalues=True, sua=True,
                 pool_momentum=0.5):
    ...
    def _treat_order_dict(self):
        ...
        # for module in self.interesting_modules:
        #     new_A[module] = self.A[(module.idx, 0)].cpu()
        #     new_G[module] = self.G[(module.idx, 0)].cpu()
        #     for k in range(1, self.nb_gpus):
        #         new_A[module] += self.A[(module.idx, k)].cpu()
        #         new_G[module] += self.G[(module.idx, k)].cpu()
        #     new_A[module] /= self.nb_gpus
        #     new_G[module] /= self.nb_gpus
        #     new_a[module] = torch.cat([self.a[(module.idx, k)].cpu() for k in range(self.nb_gpus)])
        #     new_g[module] = torch.cat([self.g[(module.idx, k)].cpu() for k in range(self.nb_gpus)])
        self.A = new_A
        self.G = new_G
        self.g = new_g
        self.a = new_a
