...
def add_conv_rows(W, n, alpha):
    w = W.contiguous().view(W.size(0), -1)
    last = w[-1, :]
    last = last + alpha*torch.abs(last)*torch.rand(last.shape).cuda()
...
def add_conv(W, n_cols, n_rows, alpha=1e-2):
    if 0 < n_cols:
        W = add_conv_cols(W, n_cols, alpha)
    if 0 < n_rows:
        W = add_conv_rows(W, n_rows, alpha)
    return W


def add_linear(W, n_cols, n_rows, alpha=1e-2):
    if 0 < n_rows:
        last = W[-1, :]
        last = last + alpha * torch.abs(last) * torch.rand(last.shape).cuda()
