...
def regularization_factor_2(C, K, G):
    ...
    factor_g = trG/G.size(0)
    res_c = ((factor_c**2)/(factor_k * factor_g))**(2/3)
    res_k = ((factor_k**2)/(factor_c * factor_g))**(2/3)
    res_g = ((factor_g**2)/(factor_k * factor_c))**(2/3)
...
class Pool_v2:
    ...
    def add(self, decomp_loss, deleted_rows_score, total):
        self.total[self.current_module] = total
        if self.current_module not in self.loss.keys():
            self.score[self.current_module] = deleted_rows_score
            self.loss[self.current_module] = decomp_loss
        else:
            self.loss[self.current_module] = self.pool_momentum*self.loss[self.current_module] +\
                                             (1 - self.pool_momentum)*decomp_loss
            self.score[self.current_module] = self.pool_momentum*self.score[self.current_module] +\
                                             (1 - self.pool_momentum)*deleted_rows_score
        if self.current_module == self.nb_modules-1:
            self.current_module = 0
            self.it += 1
        else:
    ...
    def _get_all_loss(self):
        all_loss = list(self.loss.values())
        all_loss = np.array(all_loss)
        all_loss /= all_loss.sum()
        return all_loss

    def _get_all_total(self):
        res = sum(list(self.total.values()))
        return res

    def random_pick(self):
        assert self.current_module == 0, 'Pool not completly filled!'
        n_samples = int(self.ratio*self._get_all_total())
        self.total = {}
        all_loss = self._get_all_loss()
        chosen = np.random.choice(list(self.loss.keys()), size=n_samples, p=all_loss)
        selected = {k: 0 for k in self.loss.keys()}
        for k in chosen:
            selected[k] += 1
        res = {k: {'rows': 0, 'cols': 0} for k in self.loss.keys()}
        for k in selected.keys():
            total = selected[k]
            score = self.score[k]
            nb_rows = int(score*total + 0.5)
            res[k]['rows'] = nb_rows
            res[k]['cols'] = total - nb_rows
        return res
...
def load_checkpoint_pruning(checkpoint_path, net, use_bias):
    "Be careful, work only if allow_increase was set to True during pruning"
