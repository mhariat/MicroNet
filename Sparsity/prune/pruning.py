...
class Pruner(metaclass=ABCMeta):
    ...
    def fine_tune(self, epochs, optimizer, scheduler, train_dataloader, val_dataloader, alpha=0, logger=None):
        ...
        for epoch in range(epochs):
            if self.skip:
                train_accuracy, train_loss = train_sp(model=self.model, optimizer=optimizer, use_cuda=self.use_cuda,
                                                      ...
                                                      grad_mask=self.grad_mask, log_interval=self.log_interval,
                                                      multi_gpu=multi_gpu)
                ...
                train_accuracy, train_loss = train(model=self.model, optimizer=optimizer, use_cuda=self.use_cuda,
                                                   train_dataloader=train_dataloader, epoch=epoch, multi_gpu=multi_gpu,
                                                   grad_mask=self.grad_mask, log_interval=self.log_interval)
        ...
        self._update_extra_stat({'Other/Epochs': epochs})
        self.update_it()
        torch.cuda.empty_cache()
        gc.collect()
