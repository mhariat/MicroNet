...
from utils.micronet_metric import *
import json
...
class SparsityResults:
    def __init__(self, result_dir, train_dataloader, val_dataloader, pruner, exp_name=None):
        ...
        writer_dir = '{}/writer/sparsity/{}'.format(result_dir, path_to_add)
        ...
        self.writer = SummaryWriter(writer_dir)
        checkpoint_dir = '{}/checkpoint/sparsity/{}'.format(result_dir, path_to_add)
        create_dir(checkpoint_dir)
        self.checkpoint_dir = checkpoint_dir
        stats_dir = '{}/stats/sparsity/{}'.format(result_dir, path_to_add)
        create_dir(stats_dir)
        log_dir = '{}/logs/sparsity/{}'.format(result_dir, path_to_add)
    ...
    def _compute_basic_stats(self):
        ...
        total_flops = get_total_flops(self.pruner.model, img_size, batch_size)
        inference_time = get_inference_time(self.pruner.model, img_size, batch_size)
        total_parameters = self.pruner.get_nb_parameters()
        total_parameters_per_module = self.pruner.get_nb_parameters_per_module()
        micronet_score = score(self.pruner.model, img_size, self.nb_classes, batch_size)
        stat = {
            'Indicator/total_flops': total_flops,
            'Other/total_parameters': total_parameters,
            'Other/total_parameters_per_module': total_parameters_per_module,
            'Indicator/inference_time': inference_time,
            'Performance/train_loss': train_loss,
            'Performance/train_acc': train_acc,
            'Performance/val_loss': val_loss,
            'Performance/val_acc': val_acc,
            'Ratio/prune_ratio': 0,
            'Ratio/prune_ratio_per_module': {name: 0 for name in total_parameters_per_module.keys()},
            'Performance/micronet_score': micronet_score
        }
        if self.pruner.skip:
            stat.update({'Indicator/Computation_percentage': cp.item()})
        if self.steps != 0:
            initial_nb_parameters = self.stats[0]['Other/total_parameters']
            initial_nb_parameters_per_module = self.stats[0]['Other/total_parameters_per_module']
            stat['Ratio/prune_ratio'] = (initial_nb_parameters - total_parameters)/initial_nb_parameters
            ratio_per_module = {}
            ...
            stat['Ratio/prune_ratio_per_module'] = ratio_per_module
        enable_print()
        return stat
    ...
    def _save_stats(self):
        # dumped = json.dumps(self.stats, cls=NumpyEncoder)
    ...
    def _save_model(self):
        prune_ratio = self.stats[self.steps]['Ratio/prune_ratio']
        val_acc = self.stats[self.steps]['Performance/val_acc']
        filename = 'checkpoint_{}_{:.4f}_{:.4f}_{:.4f}.pth'.format(self.file_name, val_acc, self.pruner.compression,
                                                                   prune_ratio)
