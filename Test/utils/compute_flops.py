...
def compute_conv2d_flops(module, input, output):
    ...
    vector_length = module.kernel_size[0] * module.kernel_size[1] * input_channels * (1 - sparsity)
...
def get_total_flops(model, input_res, test_dataloader):
    ...
    total_flop_mults = sum(t[0] for t in list_total) + classification_flops
    total_flop_adds = sum(t[1] for t in list_total) + residual_addition_flops

    total_flop_exponent = 0
    for module in model.modules():
        if isinstance(module, (nn.Conv2d, nn.Linear)):
            total_flop_exponent += 1
    ...
    total_flop_exponent /= test_dataloader_size

    return [total_flop_mults/1e9, total_flop_adds/1e9, total_flop_exponent/1e9]
